...
#### La taille de l’effet {.unnumbered}
...
Il est possible de faire tous les analyses de G\*power avec R, mais cela est nettement plus complexes, car il faut tous coder à la main.
...
Pour l'analyse avec R, il faut définir d'abord calculer la taille d'effet `d` pour un test de t comparant deux moyennes, puis utiliser la fonction `pwr.t.test` de l'extension `pwr`.
...
pwr.t.test(
  n = 18, d = d(u1 = 2.64, sd1 = 3.28, u2 = 3.31, sd2 = 2.79), sig.level = 0.05,
  type = "two.sample"
)
...
Étudions un peu la figure @fig-r-power.
...
**Une analyse de puissance post hoc avec la taille de l’effet observé n’est pas très utile.** On la fera plutôt pour une taille d’effet autre que celle observée quand H 0 est acceptée.
Quelle taille d’effet utiliser?
C’est la biologie du système étudié qui peut nous guider.
