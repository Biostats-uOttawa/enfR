...
#### Évaluer l’évidence {.unnumbered}

L’analyse de puissance effectuée après avoir accepté une hypothèse nulle permet de calculer la probabilité que l’hypothèse nulle soit rejetée si elle était fausse et que la taille d’effet était d’une valeur donnée.
Ce type d’analyse *a posteriori* est très commun.

#### Planifier de meilleures expériences {.unnumbered}
...
#### Estimer la "limite de détection" statistique {.unnumbered}
...
#### Le critère de décision {.unnumbered}
...
#### La taille de l’échantillon {.unnumbered}
...
#### La taille d’effet {.unnumbered}

Plus la taille d’effet est grande, plus un test a de puissance.
...
La taille d’effet est en fait une mesure du degré de fausseté de l’hypothèse nulle.
...
Il peut cependant être utilisé sous linux via Wine ou une machine virtuelle.
...
Il est possible de faire toutes les analyses de G\*power avec R, mais cela est un peu plus complexes et nécessite un peu plus de code et donc une meilleure compréhension du processus.
...
**Calcule** $\boldsymbol{\alpha}$ **et** $\boldsymbol{\beta}$ pour un rapport $\beta / \alpha$ donné, un effectif fixe, et une taille d’effet donnée.
...
**Calcule** $\boldsymbol{\alpha}$ pour $\beta$, effectif et taille d’effet donné.
En pratique, je vois peu d’utilité pour ce type de calcul.
Contactez-moi si vous en voyez une!

#### Post-hoc {.unnumbered}

**Calcule la** **puissance (1 -** $\boldsymbol{\beta}$**)** pour $\alpha$, une taille d’effet et un effectif donné.
...
### Comment calculer la taille d’effet ?

***G\*Power permet de faire une analyse de puissance pour de nombreux tests statistiques.***

L’indice de la taille d’effet qui est utilisé par G\*Power pour les calculs dépend du test.
...
G\*Power vous facilite la tâche et permet de calculer la taille d’effet en inscrivant seulement les valeurs pertinentes dans la fenêtre de calcul.
...
L’objectif de cette séance de laboratoire est de vous familiariser avec G\*Power et de vous aider à comprendre comment les quatre paramètres des analyses de puissance ($\alpha$, $\beta$, effectif et taille d’effet) sont reliés entre eux.
On examinera seulement l'un des nombreux tests : le *test de t* permettant de comparer deux moyennes indépendantes.
...
5.  Dans la fenêtre qui s’ouvre à droite, sélectionner $\boldsymbol{n1 = n2}$
...
La même analyse peut être faites en utlisant R.
D'abord, il faut calculer la taille d'effet `d` pour un test de t comparant deux moyennes, puis utiliser la fonction `pwr.t.test` du paquet `pwr` 📦.
...
pwr.t.test(n = 18, d = d(u1 = 2.64, sd1 = 3.28,
                         u2 = 3.31, sd2 = 2.79),
...
abline(v = -qc, col = "green")
lines(x, dnorm(x, mean = (3.31 - 2.64)),
      type = "l", col = "blue", lwd = 2)

# la puissance correspond à la partie bleutée
y <- dnorm(x, mean = (3.31 - 2.64))
polygon(c(x[x <= -qc], -qc), c(y[x <= -qc], 0),
        col = rgb(red = 0, green = 0.2, blue = 1, alpha = 0.5))
polygon(c(x[x >= -qc], -qc), c(dnorm(x[x >= -qc]), 0),
        col = rgb(1, 0, 0, 0.3))
polygon(c(x[x <= qc], qc), c(dnorm(x[x <= qc]), 0),
        col = rgb(1, 0, 0, 0.3))
```
...
<!--# JULIEN - J'ai essayé de régler ça mais je vois que même dans la version anglaise on a le même problème -->
...
-   Les régions ombrées en rose correspondent aux **zones de rejet de** $\boldsymbol{H_0}$ (${\alpha/2}$) <!--# JULIEN - j'ai rajouté le alpha/2 parceque je trouve que ça aide à comprendre mais enlève le si tu pense que ce n'est pas pertinent ou sujet à quiproquo -->. Si Jaynie avait obtenu une valeur de t en dehors de l’intervalle délimité par les valeurs critiques allant de -2.03224 à 2.03224, alors elle aurait rejeté $H_0$, l’hypothèse nulle d’égalité des deux moyennes. En fait, elle a obtenu une valeur de t égale à -0.5746 et conclu que la biomasse est la même dans les deux régions.
...
**Une analyse de puissance post hoc avec la taille d’effet observé n’est pas très utile.** On la fera plutôt pour une taille d’effet autre que celle observée quand $H_0$ est acceptée.
...
Quelles étaient les chances de Jaynie de détecter une différence de 2.64 $g/m^2$ entre les deux régions?
G\*Power peut nous le dire.

::: callout-caution
# Exercice

Changer la moyenne du groupe 2 à 5.28, recalculer la taille d’effet, et cliquer sur **Calculate** pour obtenir (\@ref(fig:gpower-2)) <!--# JULIEN - même problème de cross-référence ici -->.
:::
...
Cela pourrait être pour deux raisons: soit qu’il n’y a pas de différences entre les régions, ou alors parce que la précision des mesures est si faible et l’effort d’échantillonnage si limité qu’il était très peu probable de détecter même d’énormes différences.
...
### Analyse à priori - Calculer la taille de l’effectif à échantillonner

Supposons qu’on puisse défendre la position qu’une différence de biomasse observée par Jaynie entre les deux régions, $3.31- 2.64=0.67g/m^2$, soit écologiquement significative.
...
Assurez-vous que les valeurs pour les moyennes et les écarts-type soient celles obtenu par Jaynie, recalculez la taille d’effet, et inscrivez 0.8 pour la puissance.
:::
...
```{r}
pwr.t.test(power = 0.8, d = d(u1 = 2.64, sd1 = 3.28,
                              u2 = 3.31, sd2 = 2.79),
           sig.level = 0.05, type = "two.sample")
```

**Ouch!** Il faudrait échantillonner 326 ruisseaux dans chaque région!
Cela coûterait une fortune et exigerait de nombreuses équipes de travail.
...
```{r}
pwr.t.test(power = 0.95, d = d(u1 = 2.64, sd1 = 3.28,
                               u2 = 3.31, sd2 = 2.79),
           sig.level = 0.05, type = "two.sample")
```
...
```{r}
pwr.t.test(power = 0.8, n = 18,
           sig.level = 0.05, type = "two.sample")$d * sqrt((3.28^2 + 2.79^2) / 2)
```
