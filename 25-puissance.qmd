...
D'abord, il faut calculer la taille d'effet `d` pour un test de t comparant deux moyennes, puis utiliser la fonction `pwr.t.test` du paquet `pwr` :package:.
...
Étudions un peu la figure @fig-gpower-1.
...
Changer la moyenne du groupe 2 à 5.28, recalculer la taille d’effet, et cliquer sur **Calculate** pour obtenir @fig-gpower-2.
:::
...
Assurez-vous que les valeurs pour les moyennes et les écarts-type soient celles qu’a obtenu Jaynie, recalculez la taille de l’effet, et inscrivez 0.8 pour la puissance @fig-gpower-3.
:::
