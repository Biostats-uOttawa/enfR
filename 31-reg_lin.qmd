...
Comparez les diagrammes de dispersion avant et après transformation (Figures @fig-stur-2 et @fig-stur-log). Comme l’analyse de corrélation présuppose une relation linéaire entre les variables, on devrait donc privilégier l’analyse sur les données log-transformées.
...
par(mfrow = c(2, 2))
plot(RegModel.3)
sturgeon.male.subset <- subset(
  sturgeon, subset = !(rownames(sturgeon.male) %in% c("8", "24", "112")))
bptest(RegModel.3)
dwtest(RegModel.3)
