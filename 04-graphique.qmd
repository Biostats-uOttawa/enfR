...
Si nous n'avions inclus que la variable `weight` plutôt que `licornes$weight`, l'indicateur `plot()` affichera une erreur car la variable `weight` n'existe que dans le `licornes` l'objet "data frame".
...
hist(licornes$height, breaks = brk, main = "Unicorn height")
```
...
Par exemple, créons un diagramme en boîte de la variable `weight` à partir de notre `licornes` cadre de données.
...
#| out-width: 75%
#| fig-align: center
boxplot(licornes$weight, ylab = "weight (g)")
```
...
Dans le diagramme en pointillés ci-dessus, les données de la base de données `height` sont représentées le long de l'axe des x et les données sont représentées dans l'ordre dans lequel elles apparaissent dans la fonction `licornes` sur l'axe des y (les valeurs situées en haut de l'axe des y apparaissent plus tard dans la base de données et celles situées plus bas apparaissent au début de la base de données).
...
Créons un nuage de points multi-panneaux de toutes les variables numériques de notre `licornes` cadre de données.
...
La première rangée de diagrammes affiche les `height` sur l'axe des y et les variables `weight`, `mane_size`, `fluffyness` et `licornes` sur l'axe des x pour chacun des quatre graphiques respectivement.
La rangée suivante de placettes comporte `weight` sur l'axe des y et `height`, `mane_size`, `fluffyness` et `licornes` sur l'axe des x.
Nous interprétons les autres lignes de la même manière, la dernière ligne affichant la valeur `licornes` sur l'axe des y et les autres variables sur l'axe des x.
...
À titre d'exemple, voyons comment la relation entre le nombre de licornes (`licornes` ) et le `weight` de licornes change en fonction de `mane_size`.
...
Par exemple, nous pouvons examiner la relation entre `licornes` et `weight` conditionnées par le facteur `food`.
Le graphique en bas à gauche représente la relation entre `licornes` et `weight` pour les licornes de la `low` traitement alimentaire.
