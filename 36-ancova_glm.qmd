...
Commençons par examiner les données. Pour faciliter la comparaison, il serait utile de tracer la droite de régression et la trace "lowess" pour évaluerplus facilement la linéarité. On peut aussi ajouter un peu de code pour obtenir des légendes plus complètes (remarquez l’utilisation de la commande `expression()` pour obtenir des indices) :

```{r}
#| label: fig-anc1
#| warning: false 
#| message: false
#| fig-cap: Longueur des esturgeons en fonction de l'age entre mâles et femelles.
...
```
<!-- Probabilité que le terme lage*sex n’affecte pas la longueur à la fourche (i.e. que la pente ne diffère pas entre les sexes, et que la différence de taille entre les mâles et femelles ne varie pas avec l’âge) - JULIEN, il faudrait rephraser ici, ou alors enlever -->

> Notez que l'on utilise la fonction `Anova()` du paquet `car` :package: avec un "A" majuscule au lieu de la fonction native `anova()` (avec un "a" minuscule") pour obtenir les **sommes de carrés de type III (partiels)**. Ces sommes des carrés des écarts sont calculées comme si la variable était entrée la dernière dans le modèle et correspondent à la différence entre la variance expliquée par le modèle complet et par le modèle sans cette variable.
...
Vous noterez qu'il y a une observation qui a un résidu normalisé (Standardize residual) qui est élevé, i.e. une valeur extrême (cas numéro 49). Éliminez-la de l'ensemble de données et refaites l'analyse. Vos conclusions changent-elles?
...
#| warning: false
#| message: false
#| fig-cap: Longueur des esturgeons en fonction de l'age selon le site d'étude d'après `"anc3dat.csv"`
