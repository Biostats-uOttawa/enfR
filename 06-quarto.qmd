...
Maintenant, pour convertir votre fichier `.qmd` en document HTML, cliquez sur le petit triangle noir à côté de l'icône `Knit` en haut de la fenêtre source et sélectionnez `render to HTML`
...
Si vous souhaitez tricoter votre `.qmd` en un document pdf, il vous suffit de choisir `render to PDF` au lieu de `render to HTML` lorsque vous cliquez sur le bouton `knit` l'icône
